...
const SingularityMap_1 = __importDefault(require("../maps/SingularityMap"));
const DEBUG_ENABLED = true; // set to false in production build
const MAP_DATA = SingularityMap_1.default;
const PLAYER_BODY = {
    width: 20,
    height: 46,
    originX: 0.5,
...
class ArenaRoom extends colyseus_1.Room {
    ...
    onJoin(client, options) {
        ...
        // Add body for player
        this.createPhysicsBody(client.sessionId, MAP_DATA.room_boundaries['L0'], 0, PLAYER_BODY.width, PLAYER_BODY.height);
        // Add state object for sword
        this.state.objects.set(`sword_${client.sessionId}`, new ArenaRoomState_1.AbstractObject(`sword_${client.sessionId}`, MAP_DATA.room_boundaries['L0'], 0, OBJECT_BODIES['sword'].width, OBJECT_BODIES['sword'].height, OBJECT_BODIES['sword'].originX, OBJECT_BODIES['sword'].originY, 'sword'));
        // Add body for sword
        this.createPhysicsBody(`sword_${client.sessionId}`, MAP_DATA.room_boundaries['L0'], 0, OBJECT_BODIES['sword'].width, OBJECT_BODIES['sword'].height);
