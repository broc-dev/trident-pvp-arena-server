...
const arena_1 = __importDefault(require("../maps/arena"));
...
const SWORD_ATTACK_FRAME_XOFFSETS = [
    // 0,
    1,
    4,
    16,
...
class ArenaRoom extends colyseus_1.Room {
    ...
    createPhysicsBody(id, x, y, width, height) {
        this.physicsBodies[id] = this.physics.add.body(x, y, width, height);
        if (DEBUG_ENABLED) {
            this.state.hitboxDebug.set(id, new ArenaRoomState_1.HitboxDebug(id, x, y, width, height));
        }
    }
    getOtherPlayerID(sessionId) {
        let otherPlayerID = '';
    ...
    onCreate(options) {
        ...
        this.onMessage('change-stance', (client, data) => {
        ...
        this.onMessage('keyboard-input', (client, input) => {
            ...
            if (!player.isDead) {
                ...
                if (hasSword && isGrounded && doThrowAttack) {
                    ...
                    else if (player.flipX && player.velX < 0 && player.velX > -MAX_SPEED ||
        ...
        for (let y = 0; y < arena_1.default.height; y++) {
            for (let x = 0; x < arena_1.default.width; x++) {
                const px = (x * arena_1.default.tile_width);
                const py = (y * arena_1.default.tile_height);
                const i = (y * arena_1.default.width + x);
                const isBlocking = (arena_1.default.data[i] === 1);
                if (isBlocking) {
                    this.physicsMap = [
                        ...this.physicsMap,
                        this.physics.add.staticBody(px, py, arena_1.default.tile_width, arena_1.default.tile_height)
                    ];
                }
            }
        }
    ...
    moveHeldSwords() {
        this.state.players.forEach((player, playerID) => {
            ...
            if (isPlayerHoldingSword) {
                const player = this.state.players.get(playerID);
                const isSwordOutAnim = (player.anim.startsWith('sword-idle') ||
    ...
    onJoin(client, options) {
        ...
        this.state.players.set(client.sessionId, new ArenaRoomState_1.Player(options.playerName, client.sessionId, PLAYER_BODY.width, PLAYER_BODY.height));
        // Add body for player
        this.createPhysicsBody(client.sessionId, arena_1.default.room_boundaries['L0'], 0, PLAYER_BODY.width, PLAYER_BODY.height);
        // Add state object for sword
        this.state.objects.set(`sword_${client.sessionId}`, new ArenaRoomState_1.AbstractObject(`sword_${client.sessionId}`, arena_1.default.room_boundaries['L0'], 0, OBJECT_BODIES['sword'].width, OBJECT_BODIES['sword'].height, OBJECT_BODIES['sword'].originX, OBJECT_BODIES['sword'].originY, 'sword'));
        // Add body for sword
        this.createPhysicsBody(`sword_${client.sessionId}`, arena_1.default.room_boundaries['L0'], 0, OBJECT_BODIES['sword'].width, OBJECT_BODIES['sword'].height);
